...
public class KettleRecipesSupport {
    ...
    public static void init() {
        MineTweakerImplementationAPI.onReloadEvent(reloadEvent -> {
            ...
            MTUtil.wrapError(() -> loadKettle.invoke(Witchery.Recipes));
        });
    }
    ...
    public static class KettleRecipeWrapper {
        ...
        static {
            ...
            ReflectUtil.makeAccessible(power);
            ReflectUtil.makeAccessible(color);
            ReflectUtil.makeAccessible(hatBonus);
            ReflectUtil.makeAccessible(familiarType);
            ReflectUtil.makeAccessible(dimension);
            ReflectUtil.makeAccessible(inBook);
        }
        ...
        public KettleRecipeWrapper power(float powerValue) {
            MTUtil.wrapError(() -> power.setFloat(this.recipe, powerValue));
            return this;
        }

        @ZenMethod
        public KettleRecipeWrapper color(int colorValue) {
            MTUtil.wrapError(() -> color.setInt(this.recipe, colorValue));
            return this;
        }

        @ZenMethod
        public KettleRecipeWrapper hatBonus(int hat) {
            MTUtil.wrapError(() -> hatBonus.setInt(this.recipe, hat));
            return this;
        }

        @ZenMethod
        public KettleRecipeWrapper familiar(int type) {
            MTUtil.wrapError(() -> familiarType.setInt(this.recipe, type));
            return this;
        }

        @ZenMethod
        public KettleRecipeWrapper dimension(int dim) {
            MTUtil.wrapError(() -> dimension.setInt(this.recipe, dim));
            return this;
        }

        @ZenMethod
        public KettleRecipeWrapper inBook(boolean value) {
            MTUtil.wrapError(() -> inBook.setBoolean(this.recipe, value));
            return this;
        }
    }
}
